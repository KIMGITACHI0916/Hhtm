...
from commands.collect import handle_grab_command, handle_group_message
from commands.economy import get_economy_handlers
from commands.harem import get_harem_handlers
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Hello! Slave bot is online.")
...
def main():
    ...
    # ✅ Economy handlers (/daily, /wallet, /pay, /trade, /give, /market)
    for h in get_economy_handlers():
        app.add_handler(h)

    # ✅ Harem handlers (/harem, /collection, /fav)
    for h in get_harem_handlers():
        app.add_handler(h)
